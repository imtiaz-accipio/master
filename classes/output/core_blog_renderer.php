...
class core_blog_renderer extends \core_blog_renderer {
    ...
    public function render_blog_entry(blog_entry $entry) {
        ...
        if ($entryid == 'none') {
            ...
            if ( $firstentry == null ) {
                ...
                $ou = html_writer::start_tag('ul',array('class' => 'row blogUL'));
                $firstentry = true;
            }
            //echo $blogcount;
            $ou .= $this->render_blog_archive($entry);
            if ($blogcount == $totalblog) {
                $ou .= html_writer::end_tag('ul'); // ending of ul tag after the last blog for current page.
    ...
    public function render_blog_archive(blog_entry $entry) {
        ...
        $by->name = html_writer::link(new moodle_url('/user/view.php', $userurlparams), $fullname, array('style' => 'text-decoration:none; color:#cbcbcc;'));
        ...
        $ou = html_writer::start_tag('li',array('class' => 'widget col-xs-12 col-md-4'));
        ...
        $ou .= html_writer::tag('p', get_string('author', 'theme_master') . ' - ' . $by->name);
        $ou .= html_writer::link($link, get_string('readmore', 'theme_master'), array("class" => "btn btn-primary waves-effect waves-light"));
